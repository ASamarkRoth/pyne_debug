...
def _parse_decay_dataset(lines, decay_s):
    ...
    if _to_id(daughter) < 0:
        print('Negative id for daughter and re=', ident.groups())
        raise ValueError
    daughter_id = abs(_to_id(daughter))
    if daughter_id == 581400000: 
        is_check = True
    ...
    if len(parents) > 1:
        ...
        if _to_id(parents[0]) < 0:
            print('Negative id for > 1 parent and re=', ident.groups())
            raise ValueError
    else:
        pfinal = abs(_to_id(parents[0][:5]))
        if _to_id(parents[0][:5]) < 0:
            print('Negative id for 1 parent and re=', ident.groups())
            raise ValueError
    tfinal = None
    tfinalerr = None
